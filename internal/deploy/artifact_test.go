...
func TestArtifactFetcher_VerifierRejectsSignature(t *testing.T) {
	...
	fetcher := NewArtifactFetcher(&fakeArtifactVerifier{err: fmt.Errorf("signature invalid")})
	fetcher.keychain = anonymousKeychain{}
	fetcher.nameOpts = []name.Option{name.Insecure}

	dir := t.TempDir()
	err := fetcher.FetchAndVerify(context.Background(), host+"/test/app:v1.0.0-compose", "org/repo", dir)
	if err == nil {
...
func TestArtifactFetcher_WritesFilesToDestDir(t *testing.T) {
	...
	fetcher := NewArtifactFetcher(&fakeArtifactVerifier{})
	fetcher.keychain = anonymousKeychain{}
	fetcher.nameOpts = []name.Option{name.Insecure}

	dir := t.TempDir()
	if err := fetcher.FetchAndVerify(context.Background(), host+"/test/app:v1.0.0-compose", "org/repo", dir); err != nil {
		t.Fatalf("FetchAndVerify: %v", err)
	}

	got, err := os.ReadFile(dir + "/docker-compose.yml")
	if err != nil {
...
func TestArtifactFetcher_EmptyArtifact(t *testing.T) {
	...
	err := fetcher.FetchAndVerify(context.Background(), host+"/test/empty:v1.0.0-compose", "org/repo", t.TempDir())
	if err == nil {
